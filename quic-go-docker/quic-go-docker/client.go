...
func Client() {
	...
	localAddress := ":8001" // default port
	if len(os.Args) > 3 {
	...
	go func() {
		...
		peerAddr, startTime := register(&tr, remoteRelay, tlsConfDial)     // 在线程1中执行 register() 函数
	...
	go func() {
		endTime := holepunc(&tr, tlsConfListen, tlsConfDial, peerAddr) // 在线程2中使用 peerAddr 值执行 chatter 函数
		executionTime := endTime.Sub(startTime)
		log.Println("startTime", startTime)
		log.Println("endTime", endTime)
		log.Println("Hole punching Time", executionTime)
	}()
...
func register(tr *quic.Transport, remote *net.UDPAddr, tlsConf *tls.Config) (string, time.Time) {
	// 使用 QUIC 和中继连接
	//startTime := time.Now()
	conn, err := tr.DialEarly(context.Background(), remote, tlsConf, &quic.Config{Allow0RTT: true})
	if err != nil {
...
// 客户端之间的连接（打洞）
func holepunc(tr *quic.Transport, tlsConfListen *tls.Config, tlsConfDial *tls.Config, remote string) time.Time {
	...
	go func() {
		...
		for i := 0; i < 20; i++ {
			var errDial error
			conntoPeer, errDial = tr.Dial(context.Background(), remoteAddr, tlsConfDial, nil)
			if errDial == nil {
				endTime = time.Now()
				//计算打洞结束时间
				break
